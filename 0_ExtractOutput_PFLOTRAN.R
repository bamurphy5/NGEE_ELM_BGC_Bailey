...
for(variable_name in variable_list_3D){ 
  ...
  long_data <- long_data %>%
    mutate(
      polygon_type = rep(polygon_type, times = 87600), #repeats 1:7 87600 times, format "1, 2, 4, 5, 6, 7, 1, 2, 3, 4...", total length 613200
  ...
  #Trough <- array_data[1, 1:10, 1:8760]
  #var_vec <- as.vector(Trough)
...
#                names_to = "dummy",
#                values_to = "value") %>%
#   # Add columns for location and soil layer
#   mutate(variable = variable_name,
#          lat = rep(lat, each = 8760),
#          lon = rep(lon, each = 8760),
#          polygon_type = rep(polygon_type, each = 8760),
#          soil_layer = rep(soil_layers, each = 8760), 
#          date = rep(date, each = 105),
#          sec = rep(sec, each = 105)) 
...
#   summarize(value_daily = case_when(variable %in% sum_vars ~ sum(value, na.rm = TRUE),
#                                     variable %in% avg_vars ~ mean(value, na.rm = TRUE)))
...
#using Sigrid's season approach instead
snowmelt <- c(4, 5)
growing <- c(6, 7, 8, 9)
freeze <- c(10, 11)
winter <- c(1, 2, 3, 12)
