...
!             endif
   ...
   !   real(r8) :: h2osfc_tide
     ...
     ! BAM (10/14/25): Added for adaptive use of perched (zwt_perched) or main water table (zwt) for tidal forcing in permafrost areas
     integer :: jwt_perched(bounds%begc:bounds%endc)       ! index of layer above perched water table
     integer :: wt_layer                                   ! index of active water table layer (main or perched)
     real(r8) :: wt_depth                                  ! active water table depth (main or perched)
     !-----------------------------------------------------------------------
     ...
     associate(                                                    &
       ...
       ! above
       ! the water table
       ! BAM (10/14/25) also want to calc a perched version of jwt layer index based on zwt_perched instead of zwt for permafrost scenarios

       do fc = 1, num_hydrologyc
          c = filter_hydrologyc(fc)
          jwt(c) = nlevbed
          jwt_perched(c) = nlevbed
          ...
          enddo
          ! compute perched-layer index
          do j = 1,nlevbed
             if (.not.isnan(zwt_perched(c))) then
                if(zwt_perched(c) <= zi(c,j)) then
                  jwt_perched(c) = j-1
                  exit
                end if
             end if
       ...
       do fc = 1, num_hydrologyc
		 ...
		 !changed it back to 0.01 BAM 5/15/25
               ...
               !BAM (10/14/25) editing tidal forcing routine to check whether to use zwt or zwt_perched based variables so that the forcing is more representative for permafrost scenarios where zwt_perched might be much shallower than zwt
               !compute lateral subsurface flux using either zwt or zwt_perched
               wt_depth = zwt(c) !initialize defaults to the main water table 
               wt_layer = jwt(c)

               !if zwt_perched exists and is shallower than zwt, use it, if it doesn't exist (potential non-frozen soil case) it just uses the zwt-based values initialized above. Assumes zwt and zwt_perched are positive depths below surface 
               if (.not. isnan(zwt_perched(c))) then
                  if (zwt_perched(c) < zwt(c)) then
                     wt_depth = zwt_perched(c)
                     wt_layer = jwt_perched(c)
                  endif
               endif

               ka_hu = 0._r8 !BAM had to edit this too so ka_hu isn't from a different layer than what gets pulled for wt_layer
               if (wt_layer .lt. nlevbed) then
                  do j=nlevbed,wt_layer+1,-1
                  s_node = max(h2osoi_vol(c,j)/watsat(c,j), 0.01_r8)
                  s_node = min(1.0_r8, s_node)
                  s1 = 0.5_r8*(1.0+s_node)
                  s1 = min(1._r8, s1)
                  ...
                  ka_hu = ka_hu+(hksat(c,j)*s1**(2._r8*bsw(c,j)+3._r8))* &
                           dzmm(c,j)/sum(dzmm(c,wt_layer+1:nlevbed))
                  end do
               else
                  s_node = max(h2osoi_vol(c,wt_layer)/watsat(c,wt_layer), 0.01_r8)
                  s_node = min(1.0_r8, s_node)
                  s1 = 0.5_r8*(1.0+s_node)
                  s1 = min(1._r8, s1)
                  ka_hu = ka_hu+(hksat(c,wt_layer)*s1**(2._r8*bsw(c,wt_layer)+3._r8))
               end if
               !BAM I don't think these actually get used below this point
               !zwt_hu = zwt(c)
               ...
               !DMR 12/4/2015
               !if (icefrac(c,min(jwt(c)+1,nlevbed)) .ge. .10_r8) then
                  !turn off lateral transport if any ice is present at or below,
                  !qflx_lat_aqu(c) = 0._r8
               !else
                  !qflx_lat_aqu(c) =  2._r8*ka_hu * (h2osfc_tide(c)/1000._r8 - (h2osfc(c)/1000._r8 - zwt(c))) / max(dist_from_stream(c), 1.0_r8)
               !endif
               !BAM edit to use the newly defined zwt vs zwt_perched check
               if (icefrac(c,min(wt_layer+1,nlevbed)) .ge. .10_r8) then
                  qflx_lat_aqu(c) = 0._r8
               else
                  qflx_lat_aqu(c) = 2._r8 * ka_hu * (h2osfc_tide(c)/1000._r8 - (h2osfc(c)/1000._r8 - wt_depth)) / max(dist_from_stream(c), 1.0_r8)
               endif
